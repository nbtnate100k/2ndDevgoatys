...
- Nothing needs to be added to your assistant's saved prompt for scripts
  or receptionist config to work — this bot fetches your assistant's real
  prompt and injects the Telegram user's instructions directly into it for
  each call, so it works regardless of what your saved prompt says. See
  "How your custom instructions actually get used" below if you're curious.
...
## How your custom instructions actually get used

Vapi's `assistantOverrides.variableValues` (e.g. `{{callScript}}`) only
takes effect if your assistant's *saved* prompt happens to reference that
exact placeholder — if it doesn't, Vapi silently ignores the variable and
the call just uses whatever prompt is saved, with no error. That's an easy
thing to miss, so this bot doesn't rely on it.

Instead, for every call (outbound or receptionist), the bot:

1. Fetches your assistant's real, current system prompt from Vapi (`GET
   /assistant/{id}`, cached for 5 minutes so it's not refetched on every
   single call).
2. Appends the Telegram user's script (or, for receptionist calls, the
   business name / help topics / transfer instructions) onto the end of
   that real prompt.
3. Sends that combined prompt back as a `model` override **for that one
   call only** — using the `provider` and `model` values read from your
   own assistant, which is what makes this safe: those values can never
   be missing or invalid, which is what caused the original
   `assistantOverrides.model.provider` error.

Your saved assistant is never modified — this only affects the single call
it's sent with. `variableValues` (`callScript`, `businessName`, etc.) are
still sent alongside this as a harmless bonus, in case your prompt already
references them.
...
topics, and transfer settings, injected directly into the prompt as
described above — never by editing your saved assistant.
...
- **The Telegram script / business config still doesn't seem to be used**:
  check Railway's logs right after placing a call for a line starting
  with `Failed to fetch assistant config for prompt injection:` — if
  present, it'll show exactly why (usually the assistant doesn't have a
  supported `model.provider`/`model.model` set, or `VAPI_ASSISTANT_ID` is
  wrong). If that fetch fails, the bot falls back to the assistant's
  default prompt so the call still goes through rather than failing
  outright, but your custom instructions won't be applied until the fetch
  works.
- **Transfer never happens**: check the transfer trigger you picked isn't
  "Never transfer automatically" — that option deliberately withholds the
  transfer capability from the assistant entirely for that call.
...
src/assistantModel.js   Fetches/caches your assistant's real prompt and
                         builds reliable per-call instruction injections
                         (used by both vapiClient.js and webhook.js)
