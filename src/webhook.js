...
function createWebhookRouter(bot) {
  ...
  router.post('/webhook/vapi', express.json({ limit: '2mb' }), async (req, res) => {
    ...
    if (message?.type === 'assistant-request') {
      return handleAssistantRequest(res, message);
    }
 ...
 */
function handleAssistantRequest(res, message) {
  ...
  const overrides = {
    variableValues: {
      ...
      transferInstructions: buildTransferInstructions(cfg.transferTrigger, cfg.transferDetail),
    },
